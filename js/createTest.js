...
function renderQuiz(quizId) {
  ...
  fetch("https://apiquizizz.herokuapp.com/quizzes")
    ...
    .then((data) => {
      ...
      document.querySelector(".quiz-image-inner").firstElementChild.src =
        data[quizId][0].image;
    ...
    .then(() => {
      ...
      questionOptions.forEach((element, index) => {
        ...
        element.childNodes.forEach((answer, answerIndex) => {
          ...
          answer.firstChild.setAttribute(
            "name",
            quizData[quizId][index].quizNumber
          );

          // Tinh diem dung sai
          answer.firstChild.addEventListener("click", () => {
            if (
              answer.lastChild.innerText[0] ==
              quizData[quizId][index].rightAnswer
            ) {
          ...
          answer.lastChild.innerHTML =
            quizData[quizId][index].answers[answerIndex];
        });
      });
    })
    .then(() => {
