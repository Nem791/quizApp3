    ...
    fetch('http://localhost:3000/quizzes')
        ...
        .then((data) => {
            ...
            for (let index = 0; index < Object.keys(data).length; index++) {
                ...
                dropdownContent.appendChild(createATag(data[quizId][0].questionTitle, quizId));
            }
            ...
            db.collection(data[setQuizId][0].questionTitle)
                ...
                .then((querySnapshot) => {
                    querySnapshot.forEach((doc) => {
                        console.log(typeof doc.data().score);
                        // Ham board se co dang Array trong Array [[name, score, time], [name2, score2, time2]]
                        board.push([doc.data().username, doc.data().score, doc.data().time]);
                    });
                })
                .then(() => {
                    // Sap xep cao => thap 
                ...
                .then(() => {
                    ...
                    tableRow.forEach((element, index) => {
                        ...
                        element.childNodes[2].innerText = board[index][1] + ' / ' + data[setQuizId].length;
                        // Accuracy 
                        element.childNodes[3].firstChild.innerText = Number(board[index][1]) / Number(data[setQuizId].length) * 100 + '%';
                        // Time 
